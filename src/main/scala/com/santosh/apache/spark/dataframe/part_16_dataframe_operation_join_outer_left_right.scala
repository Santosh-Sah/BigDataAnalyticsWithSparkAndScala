...
object part_16_dataframe_operation_join_outer_left_right {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession
      .builder()
      .appName(name = "Dataframe Operation: join | outer, left, right")
      .master(master = "local[*]")
      .getOrCreate()
    ...
    val left_join_df_1 = order_df.join(user_df, Seq("user_id"), joinType = "left")
    ...
    val left_join_df_2 = order_df.as(alias = "o").join(user_df.as(alias = "u"), joinExprs = ($"o.user_id" === $"u.user_id"), joinType = "left")
    left_join_df_2.show(numRows = 10, truncate = false)

    val right_join_df_1 = order_df.join(user_df, Seq("user_id"), joinType = "right")
      .select(col = "user_id", cols = "order_id", "card_type", "product_category", "order_amount", "order_datetime", "user_name", "user_city")

    right_join_df_1.show(numRows = 10, truncate = false)

    import spark.implicits._
    val right_join_df_2 = order_df.as(alias = "o").join(user_df.as(alias = "u"), joinExprs = ($"o.user_id" === $"u.user_id"), joinType = "right")
